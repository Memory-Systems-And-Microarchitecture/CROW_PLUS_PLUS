...
public:
    ...
    int* ref_counters;
    //crow ++
    bool enable_crow_plus_plus = false;
    //crow ++
        ...
        cmd_trace_files(channel->children.size())
    {
        ...
        //crow ++
         enable_crow_plus_plus = configs.get_bool("enable_crow_plus_plus");
        //crow ++
        if (record_cmd_trace){
    ...
    void finish(long read_req, long dram_cycles) {
      ...
      //crow_table->store_into_file();
      //crow_table->finish_all();
      //printf("Number of Crow Add entries: %d\n",crow_table->temp_track_add_entry);
    }
        ...
        if (req->is_first_command) {
            ...
            if (req->type == Request::Type::READ || req->type == Request::Type::PREFETCH) {
                if (is_row_hit(req)) {
                    ...
                    //std::cout<<read_row_hits[coreid]<<endl;
                    //printf("read row_hits: %d\n",read_row_hits[coreid]);
...
private:
    ...
    void initialize_crow() {
        ...
        crow_table->enable_crow_plus_plus = enable_crow_plus_plus;
